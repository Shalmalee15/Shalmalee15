# Hi, I'm Shalmalee. 

### Senior Data Scientist | PhD Astrophysics | Melbourne, Australia

I build production-ready ML and AI systems that turn complex data into decisions that matter. With 8+ years across financial services and large-scale data environments, I specialise in predictive pricing, demand forecasting, and applied AI.

---

##  What I'm Working On

- 🤖 **Financial RAG Pipeline** - LangChain + HuggingFace + FAISS for intelligent document Q&A
- 🚗 **Automotive Pricing & Forecasting** - XGBoost demand forecasting + price elasticity optimisation
- 🛍️ **Customer Segmentation** - RFM analysis + K-Means clustering for targeted marketing

---

##  Tech Stack

**ML & AI**
![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![XGBoost](https://img.shields.io/badge/XGBoost-FF6600?style=flat)
![Scikit-Learn](https://img.shields.io/badge/ScikitLearn-F7931E?style=flat&logo=scikit-learn&logoColor=white)
![HuggingFace](https://img.shields.io/badge/HuggingFace-FFD21E?style=flat&logo=huggingface&logoColor=black)
![LangChain](https://img.shields.io/badge/LangChain-1C3C3C?style=flat)

**Data & Cloud**
![Spark](https://img.shields.io/badge/Apache%20Spark-E25A1C?style=flat&logo=apachespark&logoColor=white)
![Databricks](https://img.shields.io/badge/Databricks-FF3621?style=flat&logo=databricks&logoColor=white)
![Azure](https://img.shields.io/badge/Azure-0078D4?style=flat&logo=microsoftazure&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-4479A1?style=flat&logo=mysql&logoColor=white)
![Snowflake](https://img.shields.io/badge/Snowflake-29B5E8?style=flat&logo=snowflake&logoColor=white)

**Visualisation**
![PowerBI](https://img.shields.io/badge/Power%20BI-F2C811?style=flat&logo=powerbi&logoColor=black)
![Plotly](https://img.shields.io/badge/Plotly-3F4F75?style=flat&logo=plotly&logoColor=white)
![R](https://img.shields.io/badge/R-276DC3?style=flat&logo=r&logoColor=white)

---

##  Featured Projects

| Project | Description | Tech |
|---------|-------------|------|
| [🤖 Financial RAG Pipeline](https://github.com/Shalmalee15/financial-rag-pipeline) | Intelligent Q&A over financial documents using RAG | LangChain, HuggingFace, FAISS, Streamlit |
| [🚗 Automotive Pricing & Forecasting](https://github.com/Shalmalee15/automotive-pricing-forecasting) | Demand forecasting + dynamic pricing for aftermarket parts | XGBoost, SciPy, Plotly |
| [🛍️ Customer Segmentation](https://github.com/Shalmalee15/Customer-Segmentation-with-Machine-Learning) | RFM-based customer segmentation with K-Means clustering | Scikit-Learn, PySpark, R |

---

##  Experience Highlights

- **Senior Data Scientist @ Angle Auto Finance**: Predictive pricing models delivering multi-million dollar revenue impact
- **Data Scientist @ Forecast Global**: Digital twin forecasting improving airport operational efficiency by 20%
- **PhD Researcher @ Macquarie University**: ML classification on Hubble Space Telescope stellar data

---

##  Let's Connect

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=flat&logo=linkedin&logoColor=white)](https://linkedin.com/in/shalmalee-kapse)
[![GitHub](https://img.shields.io/badge/GitHub-181717?style=flat&logo=github&logoColor=white)](https://github.com/Shalmalee15)
[![Email](https://img.shields.io/badge/Email-D14836?style=flat&logo=gmail&logoColor=white)](mailto:sharma.shalmalee@gmail.com)

---

*"Turning data into decisions — one model at a time."*
